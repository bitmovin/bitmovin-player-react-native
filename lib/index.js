...
var FullscreenHandlerModule = import_react_native4.NativeModules.FullscreenHandlerModule;
...
var Uuid2 = import_react_native5.NativeModules.UuidModule;
var CustomMessageHandlerModule = import_react_native5.NativeModules.CustomMessageHandlerModule;
...
function dispatch(command, node, ...args) {
  const commandId = import_react_native6.Platform.OS === "android" ? import_react_native6.UIManager.NativePlayerView.Commands[command].toString() : import_react_native6.UIManager.getViewManagerConfig("NativePlayerView").Commands[command];
  import_react_native6.UIManager.dispatchViewManagerCommand(
    node,
    commandId,
    import_react_native6.Platform.select({ ios: args, android: [node, ...args] })
  );
}
function PlayerView({
  style,
  ...
  const nativeViewStyle = import_react_native6.StyleSheet.flatten([styles.baseStyle, style]);
  ...
  (0, import_react2.useEffect)(() => {
    player.initialize();
    const node = (0, import_react_native6.findNodeHandle)(nativeView.current);
    if (node) {
...
var NativeSubtitleView = (0, import_react_native7.requireNativeComponent)(
...
function SubtitleView(props) {
  const style = import_react_native8.StyleSheet.flatten([styles2.baseStyle, props.style]);
  return import_react_native8.Platform.OS === "android" ? /* @__PURE__ */ import_react3.default.createElement(
    NativeSubtitleView,
    {
      style,
...
var Uuid3 = import_react_native9.NativeModules.UuidModule;
...
// src/drm/index.ts
var DrmModule = import_react_native10.NativeModules.DrmModule;
var Drm = class extends NativeInstance {
  constructor() {
    ...
    __publicField(this, "onPrepareMessage", (message, assetId) => {
      const config = import_react_native10.Platform.OS === "ios" ? this.config?.fairplay : this.config?.widevine;
      if (config && config.prepareMessage) {
        DrmModule.setPreparedMessage(
          this.nativeId,
          import_react_native10.Platform.OS === "ios" ? config.prepareMessage?.(message, assetId) : config.prepareMessage?.(message)
        );
      }
    });
    /**
    ...
    __publicField(this, "onPrepareLicense", (license) => {
      const prepareLicense = import_react_native10.Platform.OS === "ios" ? this.config?.fairplay?.prepareLicense : this.config?.widevine?.prepareLicense;
      if (prepareLicense) {
...
var SourceModule = import_react_native11.NativeModules.SourceModule;
...
// src/player.ts
var PlayerModule = import_react_native12.NativeModules.PlayerModule;
...
// src/offline/offlineContentManager.ts
var OfflineModule = import_react_native13.NativeModules.BitmovinOfflineModule;
  ...
  constructor(config) {
    ...
    __publicField(this, "initialize", () => {
      ...
      if (!this.isInitialized && this.config) {
        this.eventSubscription = new import_react_native13.NativeEventEmitter(
          OfflineModule
        ).addListener(
