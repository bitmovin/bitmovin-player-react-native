...
var Uuid3 = import_react_native8.NativeModules.UuidModule;
...
// src/drm/index.ts
var DrmModule = import_react_native9.NativeModules.DrmModule;
var Drm = class extends NativeInstance {
  constructor() {
    ...
    __publicField(this, "onPrepareMessage", (message, assetId) => {
      const config = import_react_native9.Platform.OS === "ios" ? this.config?.fairplay : this.config?.widevine;
      if (config && config.prepareMessage) {
        DrmModule.setPreparedMessage(
          this.nativeId,
          import_react_native9.Platform.OS === "ios" ? config.prepareMessage?.(message, assetId) : config.prepareMessage?.(message)
        );
      }
    });
    /**
    ...
    __publicField(this, "onPrepareLicense", (license) => {
      const prepareLicense = import_react_native9.Platform.OS === "ios" ? this.config?.fairplay?.prepareLicense : this.config?.widevine?.prepareLicense;
      if (prepareLicense) {
...
var SourceModule = import_react_native10.NativeModules.SourceModule;
...
var BufferModule = import_react_native11.NativeModules.BufferModule;
...
var BitmovinCastManagerModule = import_react_native14.NativeModules.BitmovinCastManagerModule;
var BitmovinCastManager = {
   ...
   */
  isInitialized: async () => {
    if (import_react_native14.Platform.OS === "ios" && import_react_native14.Platform.isTV) {
      return false;
    }
