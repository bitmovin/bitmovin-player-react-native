...
function dispatch(command, node, ...args) {
  ...
  import_react_native3.UIManager.dispatchViewManagerCommand(
    node,
    commandId,
    import_react_native3.Platform.select({ ios: args, android: [node, ...args] })
  );
}
function PlayerView({ style, player, ...props }) {
  const nativeView = (0, import_react2.useRef)(null);
  const proxy = useProxy(nativeView);
  const nativeViewStyle = import_react_native3.StyleSheet.flatten([styles.baseStyle, style]);
...
// src/offlineContentManager.ts
var OfflineModule = import_react_native10.NativeModules.BitmovinOfflineModule;
    ...
    __publicField(this, "initialize", () => {
      ...
      if (!this.isInitialized && this.config) {
        if (this.config.listener) {
          this.eventSubscription = new import_react_native10.NativeEventEmitter(
            OfflineModule
          ).addListener(
